...
//控制图片的缩放和拖拽
function imageControl(parSelecter,childSelecter){
	if(!childSelecter) childSelecter="img:eq(0)";
	this.parentEle=$(parSelecter);
	this.currEle=$(parSelecter).find(childSelecter);
	this.sourceWidth=this.currEle.width();
	this.sourceHeight=this.currEle.height();
	this.sourceLeft=(this.parentEle.width()+20-this.currEle.width())/2;
	this.sourceTop=10;
	...
	this.restoreEle=function(thisObj){
		...
		$(".restore").bind("click",function(){
			...
			thisObj.currEle.css("left",thisObj.sourceLeft);
			thisObj.currEle.css("top",thisObj.sourceTop);
		});
	};
